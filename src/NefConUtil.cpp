...
#pragma region Print usage
    ...
    std::cout << "    --remove-class-filter      Removes a service from a device class' filter collection" << '\n';
    std::cout << "      --position               Which filter to modify (required)" << '\n';
    std::cout << "                                 Valid values include: upper|lower" << '\n';
    ...
    std::cout << "      --attempt-restart-affected Best-effort attempt to restart present devices of this class; a reboot may still be required (optional)" << '\n';
    std::cout << "      --restart-timeout        Milliseconds to wait per device restart attempt, default 10000 (optional)" << '\n';
    std::cout << "    --install-filter-driver    Ordered, race-safe INF-based filter driver install; confirms declared filter services have settled" << '\n';
    std::cout << "      --inf-path               Path to the INF file to install, absolute or relative to CWD (required)" << '\n';
    std::cout << "      --class-guid             Additional Device Class GUID to restart, on top of what the INF declares (optional; does not add extra services to the settle/health check)" << '\n';
    std::cout << "      --restart-timeout        Milliseconds to wait per device restart attempt, default 10000 (optional)" << '\n';
    ...
    std::cout << "      --stop-timeout           Milliseconds to wait for a lingering filter service to leave the SCM database before retrying a failed INF install, default 10000 (optional)" << '\n';
    std::cout << "    --uninstall-filter-driver  Ordered, race-safe class filter and driver service removal without ever leaving a dangling filter entry" << '\n';
    std::cout << "      --position               Which filter to modify (required)" << '\n';
    std::cout << "                                 Valid values include: upper|lower" << '\n';
    std::cout << "      --service-name           The driver service name to remove (required)" << '\n';
    std::cout << "      --class-guid             Device Class GUID to modify (required)" << '\n';
