...
3. After editing neflib sources, run `.\sync-local-neflib.ps1` to stamp `ports/neflib/portfile.cmake` with a fresh hash of the neflib sources — vcpkg's cache is keyed off the port files, not `neflib/`'s contents, so without this step edits would not trigger a rebuild
...
- **Required:** `--inf-path`
- **Optional:** `--attempt-restart-affected` — restart devices affected by any class filter changes the INF declares instead of requiring a reboot; add `--class-guid` to also restart devices of an extra class not declared in the INF, and `--restart-timeout` to change the per-device timeout in milliseconds (default `10000`)
...
- **Optional:** `--attempt-restart-affected` — restart present devices of `--class-guid` instead of requiring a reboot; `--restart-timeout` sets the per-device timeout in milliseconds (default `10000`)
...
# Restart devices affected by the INF's class filter changes instead of requiring a reboot
...
# Restart present devices of the class instead of requiring a reboot
