...
- **Behavior:** ACPI-enumerated devices are not restarted by default because those restart attempts can trigger a disruptive system-restart request; the policy skip is reported as informational. A filter service is only expected to reach `SERVICE_RUNNING` if a device of its class is currently present. If no device of that class is present, the service is expected to remain registered but stopped (demand-started) — that is logged, not reported as an error. A present device still requires its declared filter service to reach `SERVICE_RUNNING`; failure to verify that state reports `ERROR_SUCCESS_REBOOT_REQUIRED`. If the INF's class filter targets cannot be determined after installing it, the settle step cannot verify anything trustworthy, so it also reports `ERROR_SUCCESS_REBOOT_REQUIRED` rather than silently returning success. If the INF install itself fails because a just-uninstalled filter service is still "marked for deletion", the command waits up to `--stop-timeout` for those services to leave the SCM database and retries the install once.
